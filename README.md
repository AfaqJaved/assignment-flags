...
The backend is a Bun/Nx monorepo split into two workspace packages, following a light
...
```bash
bunx nx run-many -t build          # build both packages, in dependency order
bunx nx run-many -t lint           # eslint, both packages
bunx nx run-many -t format-check   # prettier --check, both packages
...
- **A load test script** (k6/Artillery/autocannon) against `/evaluate` — not included in
  this submission. I'd add a k6 script hitting `/evaluate` at fixed concurrency,
...
- **deps** installs every workspace's dependencies once (`bun install --frozen-lockfile`,
